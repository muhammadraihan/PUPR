        ...
        <div id="panel-1" class="panel">
            ...
            <h2>Edit <span class="fw-300"><i>{{$user->name}}</i></span></h2>
                <div class="panel-toolbar">
                    <a class="nav-link active" href="{{route('pengguna.index')}}"><i class="fal fa-arrow-alt-left">
                        </i>
            ...
            <div class="panel-container show">
                <div class="panel-content">
                    ...
                    {!! Form::open(['route' => ['pengguna.update',$user->uuid],'method' => 'PUT','class' =>
                    ...
                    <div class="form-group col-md-4 mb-3">
                        {!! Form::label('jabatan', 'Jabatan', ['class' => 'required form-label']) !!}
                        {!! Form::select('jabatan', $jabatans, $user->jabatan->nama, ['class' => 'select2 form-control'.($errors->has('jabatan') ? 'is-invalid':''), 'required'
                        => '']) !!}
                        @if ($errors->has('jabatan'))
                        <div class="help-block text-danger">{{ $errors->first('jabatan') }}</div>
                        @endif
                    </div>
                    <div class="form-group col-md-4 mb-3">
                        {!! Form::label('satker', 'Satker', ['class' => 'required form-label']) !!}
                        <select class="select2 form-control @if ($errors->has('satker')) is-invalid @endif" required id="satker" name="satker">
                        <option value="{{$user->satker_id}}">{{$user->satker->nama." - Wilayah"." ".$user->satker->wilayah}}</option>
                            @foreach ($satkers as $satker)
                            <option value="{{$satker->id}}">{{$satker->nama." - Wilayah"." ".$satker->wilayah}}</option>    
                            @endforeach
                          </select>
                        @if ($errors->has('satker'))
                        <div class="help-block text-danger">{{ $errors->first('satker') }}</div>
                        @endif
                    </div>
                </div>
                <div
